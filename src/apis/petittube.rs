...
pub async fn random_video(http_client: &reqwest::Client) -> Result<String, CommandError> {
    ...
    let body = http_client.get("https://petittube.com").send().await?.server_error()?.text().await?;
    let index = body.find(YOUTUBE_EMBED).ok_or(CommandError::Custom("YOUTUBE_EMBED not found".into()))?;
    let identifier: String = body[index + YOUTUBE_EMBED.len()..].chars().take_while(|&c| c != '?').collect();
