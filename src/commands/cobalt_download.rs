...
pub const COBALT_URL: &str = "http://localhost:9000/";
...
async fn get_urls(
    ...
    match cobalt::query(http_client.clone(), url, audio_only).await {
        Ok(urls) => Ok((Cow::Borrowed(COBALT_URL), urls)),
        Err(err) => {
            // Try querying the selected instance
            cobalt::query(http_client, url, audio_only)
                .await
                .map(|urls| (Cow::Borrowed(COBALT_URL), urls))
                .map_err(|_e| match err {
